...
void do_bgfg(char **argv) 
{
    if (!strcmp(argv[0], "bg")) {
        ...
        if (argv[1] == NULL) {
            ...
            int num = atoi(argv[1]);
            if ((pjob = getjobpid(jobs, num)) == NULL) {
                printf("(%d): No such process\n", num);
                return;
            }
        } else if (argv[1][0] == '%' && isnum(argv[1]+1)) {
            int num = atoi(argv[1]+1);
            if ((pjob = getjobjid(jobs, num)) == NULL) {
        ...
        int jid = pjob->jid;
        Kill(-pid, SIGCONT);
        printf("[%d] (%d) %s", jid, pid, pjob->cmdline);
        pjob->state = BG;
    } else {
        struct job_t *pjob;
        if (argv[1] == NULL) {
...
void sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
        ...
        if (WIFEXITED(status)) {
            ...
            struct job_t *pj = getjobpid(jobs, pid);
            pj->state = ST;
        }
...
int Sigprocmask(int how, sigset_t *set, sigset_t *oldset) {
    int err;
    if ((err = sigprocmask(how, set, oldset)) < 0)
